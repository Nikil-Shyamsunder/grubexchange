   ...
   <div class="container">
	...
	<h2>Grub you want: </h2>
		<table class="table-striped table-bordered">
			...
			<tbody>
					<tr>
						<td>${grubWanted.name}</td>
						<td>${grubWanted.description} </td>
						<td><c:if test="${grubWanted.containsPeanuts == true}"> <a>Peanuts,</a> </c:if> <c:if test="${grubWanted.containsEggs == true}"> <a>Egg,</a> </c:if> <c:if test="${grubWanted.containsCowsmilk == true}"> <a>Cow's Milk,</a> </c:if> <c:if test="${grubWanted.containsTreenuts == true}"> <a>Treenuts,</a> </c:if> <c:if test="${grubWanted.containsShellfish == true}"> <a>Shellfish,</a> </c:if> <c:if test="${grubWanted.containsWheat == true}"> <a>Wheat,</a> </c:if> <c:if test="${grubWanted.containsSoy == true}"> <a>Soy</a> </c:if></td>		
					</tr>
			</tbody>
		</table>
		...
		<h2>What grub would you like to offer? </h2>
			...
			<tbody>
				<c:forEach items="${user.myGrubs}" var="grub">
					<tr>
						...
						<td><button type="button" class="mb-1 btn btn-green" onclick="window.location.href = 'ConfirmGrubController?idgrub=${grub.idgrub}';">Confirm</button></td>
					</tr>					
				</c:forEach>
			</tbody>
		</table>
